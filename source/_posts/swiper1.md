...
function swiper(params) {
  ...
  function main(width, height) {
    ...
    function getDragDistance() {
      ...
      if (!isLoop) {
        if (endDistance - startDistance > 0 && index === 0) {
          ...
          dragDistance =
            moveDistance +
            (endDistance - startDistance - (endDistance - startDistance) * 0.6);
        } else if (
          endDistance - startDistance < 0 &&
          index === nodeItems.length - 1
        ) {
          // console.log("到达最后");
          dragDistance =
            moveDistance +
    ...
    function slideMove(slideDistance) {
      ...
      if (pagination) {
        nodePagination.querySelector(classNames[4]).className =
          classNames[3].slice(1);
    ...
    function judgeMove() {
      ...
      if (endDistance < startDistance) {
        ...
        if (judgeTouch(-range)) {
          ...
          if (!isLoop && moveDistance === -(nodeItems.length - 1) * range)
            return backLocation();
          index += 1;
  ...
  function outputPagination() {
    ...
    nodePagination
      .querySelector(classNames[3])
      .classList.add(classNames[4].slice(1));
  }
